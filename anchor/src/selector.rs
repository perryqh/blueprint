...
/// A [W3C TextQuoteSelector](https://www.w3.org/TR/annotation-model/#text-quote-selector):
/// the quoted text plus up to [`CONTEXT_UNITS`](crate::CONTEXT_UNITS) code units
/// of surrounding context used to disambiguate repeated quotes.
///
/// Lives in this crate rather than the daemon because [`resolve`](crate::resolve)
/// needs it and the wasm build has to carry it across the boundary. The daemon
/// re-exports it from `src/selector.rs`, so `crate::selector::TextQuoteSelector`
...
mod tests {
    ...
    /// [`resolve`](crate::resolve) treats both as "no context recorded", so this
